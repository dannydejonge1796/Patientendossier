...
// Add lblBack to lblBackContainer
    ...
    private ScrollPane getProfilePane()
    {
        ...
        if (this.care != null) {
            ...
            Button btnUnAuthorize = new Button();
            btnUnAuthorize.getStyleClass().add("btnPrimary");
            //Set font awesome icon as graphic
            btnUnAuthorize.setGraphic(GlyphsDude.createIcon(FontAwesomeIcon.MINUS));
            ...
            //Maak een VBox aan
            HBox hBoxAuthBottom = new HBox();
            hBoxAuthBottom.setSpacing(10);
            ...
            hBoxAuthBottom.getChildren().add(comboAuthorizeCare);

            //Maak een knop om de zorgverlener een machtiging te geven en voeg deze toe
            Button btnAuthorize = new Button();
            btnAuthorize.getStyleClass().add("btnPrimary");
            //Set font awesome icon as graphic
            btnAuthorize.setGraphic(GlyphsDude.createIcon(FontAwesomeIcon.PLUS));
            hBoxAuthBottom.getChildren().add(btnAuthorize);

            //Voeg de vbox toe aan de HBox
            hBoxBottom.getChildren().add(hBoxAuthBottom);
